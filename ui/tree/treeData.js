// Загружает данные групп и оборудования для дерева
export async function loadTreeData() {

  /*
  Параллельно загружает группы, оборудование и типы оборудования.
  Создаёт nodeTypeMap для отображения названия типа.
  Создаёт groupMap, чтобы быстро связывать группы с родителями.
  Строит иерархию групп.
  Добавляет оборудование в соответствующую группу.
  Отдельно собирает оборудование без группы.
  Возвращает готовый массив данных для Wunderbaum.
  */
